...
1. **근거가 있으면 근거 있는 것만 싣는다** — 논문 Methods에 제품 코드·브랜드 구매가 적힌 논문.
2. 근거 있는 논문이 **하나도 없을 때에만** 같은 배합·같은 사양의 대표 논문으로 최대 3편까지 채운다.
3. 제품과 무관한 논문(브랜드에서 시약만 산 논문 등)은 넣지 않는다.
4. 아무것도 없으면 섹션을 아예 내지 않는다.
