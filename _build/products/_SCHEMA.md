...
"safety": {
  "heading": "안전 정보",
  "rows": [
    ["물성",            "인화점 -2℃(CC) (추정)<br>성상 무색 투명 액체"],
    ["규격",            "H2O 20 ppm max"],
    ["유해·위험문구(H)", "H225, H272, H301+H311, H314"],
    ["그림문자",        "@image"],
    ["예방조치(P)",     "P201, P202, P210"],
    ["저장·응급조치(P)", "P403 + P233"],
    ["폐기(P)",         "P501"],
    ["보관 조건",       "상온 · 차광 · 건조 환경에서 미개봉 6개월"]
  ],
  "pictograms": ["ghs06", "ghs08", "ghs05"],
  "note": "※ 취급 전 MSDS를 확인하십시오."
}
```

- 라벨 8종은 위 문구로 고정. 값은 제조사 표기 그대로 두고 서술형(성상·보관조건)만 한글로 옮긴다.
- `@image` 자리에 `pictograms` 의 아이콘이 들어간다. 코드는 UN GHS 표준 번호(`ghs01`~`ghs09`),
  아이콘은 `/img/ghs/<code>.png`, 한글 이름은 `build.py` 의 `PP_GHS`. 브랜드별로 이미지를 따로 만들지 않는다.
- 제조사가 GHS를 안 실었으면 `safety` 자체를 넣지 않는다 (빌드가 warn 으로 알려 준다).
