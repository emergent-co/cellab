#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""membrane-cell → 모델별 페이지 5개 생성 (product-upload 스킬 구조)"""
import io, os, re, json, sys

BASE = os.path.expanduser('~/mnt/rndsetup_homepage/brands/gaossunion')
SRC = io.open(os.path.join(BASE, 'membrane-cell/index.html'), encoding='utf-8').read()
...
for l in io.open(SQL, encoding='utf-8').read().split('\n'):
    f = fld(l)
    if f and f[0].startswith('GU-MBCELL'):
...
CROSS = ('<p class="pkg-note" style="margin:6px 0 14px">격막 교환형 전해셀 시리즈: '
 '<a href="/brands/gaossunion/membrane-cell-c007/">C007 2H형</a> · '
 '<a href="/brands/gaossunion/membrane-cell-c007-10/">C007-10 소용량 H형</a> · '
 '<a href="/brands/gaossunion/membrane-cell-c007-11/">C007-11 H형 플로우</a> · '
 '<a href="/brands/gaossunion/membrane-cell-c008/">C008 3H형</a> · '
 '<a href="/brands/gaossunion/membrane-cell-c008-10/">C008-10 소용량 3H형</a></p>')
