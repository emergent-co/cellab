...
## 4. 구조 요약 (2026-08 기준)
- **NAV 5개**(2026-08): 툴·실험장비(`/product/`, sub: 통합 카탈로그·메뉴얼) · 소재(`/materials/`) · 셋업 사례(`/blog/`) · 유용한 정보(sub: 제품 정보 `/info/`·배터리 사전 `/wiki/`) · 문의하기(`/contact/`, sub: 문의하기(챗)·FAQ·회사소개). 홈 밴드 CTA(재료·소모품→/materials/, 툴·장비→/product/)와 연결. 상단 가로 메뉴바(스크롤 시 반투명 고정) + 모바일 드로어. 규칙: **하위 메뉴가 있는 상위 메뉴는 noclick**, 첫 하위탭 href = 대표 href.
...
- **유용한 정보(2026-08 신설)**: NAV '에너지 랩 A to Z' 폐지 → '유용한 정보'(noclick, sub: 제품 정보 `/info/` · 배터리 사전 `/wiki/`). `/blog/battery/` 커리큘럼은 삭제+301→`/wiki/`.
...
- **콘텐츠 허브 = `/blog/` "셋업 사례"** — 논문 셋업·가이드·용어사전·트러블슈팅·도입 사례를 3열 카드로 통합. `/setups/` 인덱스는 301→`/blog/` (세부 사례 글 URL은 유지). 공정 허브: battery(커리큘럼)·deposition·heat-treatment·oxidation.
...
- **기준은 스킬이 아니라 빌드 린터가 지킨다.** 다른 대화창에서 올리든, 스킬이 발동하지 않든,
  `_build/build.py` 의 린터가 `brands/*/*/index.html` 전부를 훑어 신규 위반이면 빌드를 세운다.
  - **차단** — 잘못 넣은 것과 빠뜨린 것 모두. `NO_SAFETY`(화학품인데 안전 정보 없음)가 여기 포함된다.
  - **경고만** — `LINT_SOFT` 에 든 코드. 기준선이 아직 안 잡힌 것을 잠시 두는 자리이며 지금은 비어 있다.
- **자료 출처는 페이지마다 손으로 적지 않는다.** 문구는 `_build/brands.json` 의 `source.url` 한 곳이고,
  `_ops/build_web.py` 의 `source_line()` 과 `_ops/hench/hench_common.py` 의 `_source_line()` 이 자동으로 붙인다.
  값이 비면 줄을 만들지 않는다 — 없는 출처를 지어내지 않기 위해서다(린터 `NO_SOURCE` 가 그대로 잡는다).
  - 브랜드마다 다른 규칙(수집 방법·함정·제품군 필수사양)은 `_build/brands.json` 한 곳에 적는다.
    페이지를 고치기 전에 여기부터 읽는다.
  - 기존 부채는 `_build/lint_allow.txt` 에 이유와 함께 유예한다. **새 위반은 여기 추가하지 말고 고친다.**

- **제조사 원문의 중국어는 반드시 한글로 옮긴다.** 빌드 린터 `CJK_BODY` 가 본문에 남은 간체를 잡아 빌드를 세운다.
  - 두두켐 용어 사전은 `_ops/dodochem_zh_ko.py` 한 곳이다. 새 표현이 나오면 사전에만 추가하고 페이지를 직접 고치지 않는다.
  - 예외는 제조사 상호(`天津恒创立达`·`合肥原位科技`)와 한국어 한자어(`純水`·`濃酸`)뿐이다 — `LINT_CJK_OK`.
- **제조사 카탈로그가(CNY)를 화면에 싣지 않는다.** 규격 칸에 `220.0~680.0套` 같은 CNY 범위를 넣으면
  판매가를 나누어 매입 조건을 역산할 수 있다. 린터 `CNY_LEAK` 가 잡는다.
  규격은 반드시 실제 판매 규격(`50 g~1000 g`)에서 만든다.
