...
- **안전 정보(GHS)는 제조사 표를 값 그대로 싣는다.** H·P 코드와 수치·단위는 번역·재배열하지 않고, 원문에 없는 경고문을 지어내 붙이지 않는다.
  - 라벨만 한글로 고정한다 — `물성` / `규격` / `유해·위험문구(H)` / `그림문자` / `예방조치(P)` / `저장·응급조치(P)` / `폐기(P)` / `보관 조건`.
  - 서술형 값(성상·보관조건)만 한글로 옮긴다. 그 외 값은 원문 표기를 유지한다.
  - 그림문자는 제조사 이미지를 잘라 쓰지 말고 `safety.pictograms` 에 GHS 표준 코드(`ghs01`~`ghs09`)를 나열한다. 아이콘 `img/ghs/*.png` 한 벌을 전 브랜드가 공유하고, 한글 이름은 `build.py` 의 `PP_GHS` 가 붙인다.
  - 제조사가 GHS를 공개하지 않은 제품은 `safety` 를 비워 둔다. 빌드가 `원문 미수집: 안전정보(GHS)` 로 warn 한다 — 지어내지 않는다.
